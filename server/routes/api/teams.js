      ...
      const csvData = await Promise.all(
        teams.map(async (team) => {
          ...
          if (team.team_project_preferences[0] != null) {
            const projectIds = team.team_project_preferences.map(
              (project) => project._id
            );
...
app.post("/", async (req, res) => {
  ...
  if (!team.is_public) {
    ...
    if (!team.team_password || team.team_password === "") {
      res
        .json({
          msg: "To create a private team, you must include a team password in the request.",
        })
        .status(400);
      return;
    }
...
app.put("/team_submit/:id", async (req, res) => {
  try {
    ...
    const post = await Team.findById(
      req.params.id,
      "team_number timings members team_project_preferences"
    )
      .lean()
      .exec();
    ...
    for (let i = 0; i < updatedTeam.members.length; i++) {
      ...
      try {
        ...
        res
          .status(404)
          .json(`Error sending email to ${member.email}: ${error}`);
        return;
      }
    }
    res.send(
      "Congratulations, you submitted your team! All members should have received an email confirming the project preferences."
    );
  } catch (error) {
...
async function displayTeamData(post) {
  ...
  display +=
    "Your team number is: " +
    post.team_number +
    ". Remember that for future presentations.\n" +
...
app.put("/:id", (req, res) => {
  Team.findByIdAndUpdate(req.params.id, req.body)
    .then((team) =>
      res.json({ msg: "Updated team " + team.id + " successfully" })
    )
    .catch((err) => res.status(400).json({ error: err }));
});
...
app.delete("/:id", (req, res) => {
  Team.findByIdAndRemove(req.params.id, req.body)
    .then((team) =>
      res.json({ mgs: "Team " + team.id + " deleted successfully" })
    )
...
app.put("/join/:id", auth, async (req, res) => {
  ...
  if (!team.is_public) {
    ...
    if (!bcrypt_result) {
      res
        .status(400)
        .json({ error: "You have entered an incorrect team password." });
      return;
    }
  }
  ...
  if (user.team && user.team != team.id) {
    ...
    if (team_without_user.length == 0) {
      ...
      old_team
        ...
        .then((old_team) =>
          console.log(
            "Removed user " + req.body.user_id + " from team " + old_team.id
          )
        );
    }
  }
  ...
  user
    .save()
    .then((user) =>
      console.log("Updated user " + user.id + " to store team ID " + team.id)
    );
  ...
  team.save();
  res.json({
    mgs: "User " + req.body.user_id + " successfully joined team " + team.id,
  });
});
